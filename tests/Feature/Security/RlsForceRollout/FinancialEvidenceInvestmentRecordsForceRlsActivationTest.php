...
/**
 * FinancialEvidenceInvestmentRecordsForceRlsActivationTest —
 * FirmsVault Live Integrations, Checkpoint 4 ("Plaid financial
 * evidence add-on"). Modeled on
 ...
 */
class FinancialEvidenceInvestmentRecordsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_24_180012_prepare_row_level_security_and_force_rls_on_financial_evidence_investment_records_table.php';

    private const TABLE_MIGRATION_PATH = 'database/migrations/2026_09_24_180011_create_financial_evidence_investment_records_table.php';

    private const POLICY_NAME = 'financial_evidence_investment_records_tenant_isolation';

    private const TABLE = 'financial_evidence_investment_records';
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['plaid_security_id' => 'reassigned']));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('plaid_security_id'));
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceInvestmentRecord)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceInvestmentRecord::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, FirmIntegration $connection): array
    {
        return [
            ...
            'record_type' => 'holding',
            'plaid_external_id' => 'sec_'.Str::random(16),
            'plaid_security_id' => 'sec_'.Str::random(8),
            'plaid_investment_transaction_id' => null,
            'raw_json' => json_encode(['security_id' => 'sec_'.Str::random(8)]),
            'created_at' => now(),
            'updated_at' => now(),
        ];
    }
