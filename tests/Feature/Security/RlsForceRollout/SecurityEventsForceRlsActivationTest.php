...
class SecurityEventsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_fifty_two_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['ai_retrieval_indexes', 'deployment_configs', 'firm_ai_settings', 'customer_success_health_scores', 'security_events']);
        ...
        $this->assertSame(56, count($actuallyForced), 'Exactly fifty-two prepared tables must be FORCE RLS enabled after Section 39A-3L, Phase B6, Checkpoint 34 — no more, no less.');
    ...
    {
        $coverage = new RowLevelSecurityCoverageMappingService();
        $forced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['ai_retrieval_indexes', 'deployment_configs', 'firm_ai_settings', 'customer_success_health_scores', 'security_events']);
