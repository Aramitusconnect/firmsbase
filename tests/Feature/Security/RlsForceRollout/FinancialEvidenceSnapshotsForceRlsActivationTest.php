...
/**
 * FinancialEvidenceSnapshotsForceRlsActivationTest — FirmsVault Live
 * Integrations, Checkpoint 4. Modeled directly on
 * Tests\Feature\Integrations\IntegrationSyncCursorsForceRlsActivationTest.
 */
class FinancialEvidenceSnapshotsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_25_190010_prepare_row_level_security_and_force_rls_on_financial_evidence_snapshots_table.php';

    private const POLICY_NAME = 'financial_evidence_snapshots_tenant_isolation';

    private const TABLE = 'financial_evidence_snapshots';
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['report_version' => 99]));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('report_version'));
        $this->assertNotSame(99, $reReadAsFirmB);
    }
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceSnapshot)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceSnapshot::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, Matter $matter, FirmUser $firmUser): array
    {
        return [
            ...
            'consent_id' => null,
            'authorized_source_json' => json_encode(['firm_integration_ids' => []]),
            'authorized_account_ids_json' => json_encode([]),
            'date_range_start' => now()->subYear()->toDateString(),
            'date_range_end' => now()->toDateString(),
            'retrieved_record_refs_json' => json_encode([]),
