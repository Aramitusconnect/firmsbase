...
class TimeEntriesForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_thirty_nine_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        // or weakened.
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['time_entries', 'payment_plans', 'payment_plan_events', 'notification_events', 'contacts']);
        $actuallyForced = [];
        ...
        $this->assertSame(43, count($actuallyForced), 'Exactly thirty-nine prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 21 — no more, no less.');
    ...
    public function test_no_unrelated_prepared_table_became_force_enabled(): void
    {
        ...
        $forced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['time_entries', 'payment_plans', 'payment_plan_events', 'notification_events', 'contacts']);
        foreach ($coverage->preparedTables() as $table) {
