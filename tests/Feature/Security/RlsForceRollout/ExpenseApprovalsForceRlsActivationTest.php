...
/**
 * ExpenseApprovalsForceRlsActivationTest — proves the FORCE ROW LEVEL
 * SECURITY activation for expense_approvals (database/migrations/
 * 2026_08_27_950022_prepare_row_level_security_and_force_rls_on_
 * expense_approvals_table.php) is permanently active and behaves
 * correctly. This is the FIFTH checkpoint of this batch's 7-table
 ...
 *
 * expense_approvals is append-only BY CONVENTION at the writer-service
 * level, and — as of this same batch — also by a real, enforced
 * ExpenseApproval::booted() model guard (mirroring AiApprovalEvent::
 * booted() exactly; see the model and migration's own docblocks for
 * why this migration's WITH CHECK clause governs INSERT-time firm
 * ownership only and is explicitly NOT the append-only enforcement
 * mechanism). Per §8.2 of this batch's design, this file additionally
 * proves (a) that RLS alone does NOT deny UPDATE/DELETE (isolating the
 * RLS proof from the model guard, via a raw DB::table() bypass of
 * Eloquent), and (b) that the new booted() guard DOES deny it once
 * Eloquent is back in the loop.
 *
 * Known, stated (not hidden) residual gap: this migration does NOT
 * close the transitive cross-firm foreign-key gap between
 * expense_approvals.firm_id and the real firm_id of the expenses row
 * expense_id points at — see the migration's own docblock, part (c).
 */
class ExpenseApprovalsForceRlsActivationTest extends TestCase
{
    use EvaluatesHistoricalCheckpointScope;
    use RefreshDatabase;

    private const MIGRATION_PATH = 'database/migrations/2026_08_27_950022_prepare_row_level_security_and_force_rls_on_expense_approvals_table.php';
    ...
    private function rowAttributes(Firm $firm, Expense $expense): array
    {
        return [
            'uuid' => (string) Str::uuid(),
            'firm_id' => $firm->id,
            'expense_id' => $expense->id,
            'status' => ExpenseApprovalStatus::Pending->value,
    ...
    public function test_expense_approvals_is_discovered_by_the_forced_tables_registry(): void
    {
        $coverage = new RowLevelSecurityCoverageMappingService;

        $this->assertContains('expense_approvals', $coverage->forcedTables());
    }

    public function test_expense_approvals_has_row_level_security_enabled(): void
    {
        $row = DB::selectOne("select relrowsecurity from pg_class where relname = 'expense_approvals'");

        $this->assertNotNull($row);
        $this->assertTrue((bool) $row->relrowsecurity);
    }

    public function test_expense_approvals_has_permanent_force_row_level_security_enabled(): void
    {
        $row = DB::selectOne("select relforcerowsecurity from pg_class where relname = 'expense_approvals'");
    ...
    public function test_the_policy_has_both_an_explicit_using_and_with_check_clause(): void
    {
        $row = DB::selectOne(
            "select pg_get_expr(polqual, polrelid) as using_expr, pg_get_expr(polwithcheck, polrelid) as with_check_expr
             from pg_policy where polrelid = 'expense_approvals'::regclass and polname = 'expense_approvals_tenant_isolation'"
        );
    ...
    public function test_missing_tenant_context_cannot_read_expense_approvals(): void
    {
        $firm = Firm::factory()->create();
        ExpenseApproval::factory()->forExpense(
            $this->runWithFirmContext($firm, fn () => Expense::factory()->forFirm($firm)->status(ExpenseStatus::Submitted)->create())
        )->create();

        (new TenantContextService)->clearDatabaseTenantContext();

        $this->assertSame(0, ExpenseApproval::withoutGlobalScopes()->count());
    }

    public function test_missing_tenant_context_cannot_insert_expense_approvals(): void
    {
        $firm = Firm::factory()->create();
        $expense = $this->runWithFirmContext($firm, fn () => Expense::factory()->forFirm($firm)->status(ExpenseStatus::Submitted)->create());

        (new TenantContextService)->clearDatabaseTenantContext();

        $this->expectExceptionMessageMatches('/row-level security policy/');

        DB::table('expense_approvals')->insert($this->rowAttributes($firm, $expense));
    }
    ...
    public function test_firm_a_context_can_read_its_own_expense_approval(): void
    {
        $firmA = Firm::factory()->create();
        $approvalA = $this->runWithFirmContext($firmA, function () use ($firmA) {
            $expense = Expense::factory()->forFirm($firmA)->status(ExpenseStatus::Submitted)->create();

            return ExpenseApproval::factory()->forExpense($expense)->create();
        });

        $visibleIds = $this->runWithFirmContext(
            $firmA,
            fn () => ExpenseApproval::withoutGlobalScopes()->pluck('id')->all(),
        );

        $this->assertSame([$approvalA->id], $visibleIds);
    }

    public function test_firm_a_context_cannot_read_firm_b_expense_approval(): void
    {
        $firmA = Firm::factory()->create();
        ...
        $approvalB = $this->runWithFirmContext($firmB, function () use ($firmB) {
            $expense = Expense::factory()->forFirm($firmB)->status(ExpenseStatus::Submitted)->create();
        ...
        $this->assertNotContains($approvalB->id, $visibleIds);
    }

    public function test_firm_a_context_can_insert_a_valid_expense_approval(): void
    {
        $firmA = Firm::factory()->create();
        $expense = $this->runWithFirmContext($firmA, fn () => Expense::factory()->forFirm($firmA)->status(ExpenseStatus::Submitted)->create());

        $insertedId = $this->runWithFirmContext($firmA, fn () => DB::table('expense_approvals')->insertGetId($this->rowAttributes($firmA, $expense)));

        $this->assertIsInt($insertedId);
    }

    /**
     * §8.2: proves RLS alone does NOT deny UPDATE for this table — the
     * append-only guarantee is an application-layer (model) mechanism,
     * not an RLS one. Update via the raw query builder (bypassing
     * Eloquent, whose own booted() guard would throw first regardless)
     * isolates this proof.
     */
    public function test_firm_a_cannot_update_firm_b_expense_approval(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $approvalB = $this->runWithFirmContext($firmB, function () use ($firmB) {
        ...
        $affected = $this->runWithFirmContext($firmA, function () use ($approvalB) {
            return DB::table('expense_approvals')->where('id', $approvalB->id)->update(['reason' => 'attempted cross-firm edit']);
        });

        $this->assertSame(0, $affected, 'No rows should be visible/updatable — Firm A must not be able to update Firm B\'s expense_approvals row.');

        $reReadAsFirmB = $this->runWithFirmContext(
            $firmB,
            fn () => ExpenseApproval::withoutGlobalScopes()->find($approvalB->id),
        );
    ...
    public function test_firm_a_cannot_delete_firm_b_expense_approval(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $approvalB = $this->runWithFirmContext($firmB, function () use ($firmB) {
        ...
        $this->runWithFirmContext($firmA, function () use ($approvalB) {
            DB::table('expense_approvals')->where('id', $approvalB->id)->delete();
        });
        ...
        $this->assertNotNull($reReadAsFirmB, 'Firm A context must not be able to delete Firm B expense_approvals.');
    }

    public function test_firm_a_cannot_insert_an_expense_approval_claiming_firm_b_ownership(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $expenseB = $this->runWithFirmContext($firmB, fn () => Expense::factory()->forFirm($firmB)->status(ExpenseStatus::Submitted)->create());

        $this->expectExceptionMessageMatches('/row-level security policy/');

        $this->runWithFirmContext($firmA, fn () => DB::table('expense_approvals')->insert($this->rowAttributes($firmB, $expenseB)));
    }
    ...
    public function test_ownership_cannot_be_reassigned_across_firms(): void
    {
        ...
        $this->runWithFirmContext($firmA, function () use ($approvalA, $firmB) {
            DB::table('expense_approvals')->where('id', $approvalA->id)->update(['firm_id' => $firmB->id]);
        });
    }
    ...
    // ---------------------------------------------------------------
    // Append-only enforcement (§8.2) — a real, enforced model guard as
    // of this same batch, deliberately independent of RLS.
    // ---------------------------------------------------------------

    /**
     * The new ExpenseApproval::booted() guard (added in this same
     * batch, mirroring AiApprovalEvent::booted() exactly) DOES deny
     * update/delete once Eloquent is in the loop.
     */
    public function test_updating_an_approval_still_throws_the_append_only_guard_under_force(): void
    {
        $firm = Firm::factory()->create();
        $approval = $this->runWithFirmContext($firm, function () use ($firm) {
            $expense = Expense::factory()->forFirm($firm)->status(ExpenseStatus::Submitted)->create();
        ...
        $this->runWithFirmContext($firm, fn () => $approval->update(['reason' => 'changed']));
    }

    public function test_deleting_an_approval_still_throws_the_append_only_guard_under_force(): void
    {
        $firm = Firm::factory()->create();
        $approval = $this->runWithFirmContext($firm, function () use ($firm) {
            $expense = Expense::factory()->forFirm($firm)->status(ExpenseStatus::Submitted)->create();
        ...
        $this->runWithFirmContext($firm, fn () => $approval->delete());
    }

    // ---------------------------------------------------------------
    // ExpenseApprovalService writer regression proofs
    // ---------------------------------------------------------------

    public function test_approve_succeeds_under_force_and_clears_context(): void
    {
        $firm = Firm::factory()->create();
        ...
        $this->assertNoDatabaseTenantContext('recordDecision() must clear its own internal context wrap after success.');

        $reRead = $this->runWithFirmContext($firm, fn () => [
            'approval' => ExpenseApproval::withoutGlobalScopes()->find($approval->id),
            'expense' => Expense::withoutGlobalScopes()->find($expense->id),
        ]);

        $this->assertNotNull($reRead['approval'], 'recordDecision() must genuinely persist an expense_approvals row.');
        $this->assertSame(ExpenseStatus::Approved, $reRead['expense']->status, 'recordDecision()\'s BOTH writes (approval create + expense status update) must take effect under the SAME shared wrap.');
    }

    public function test_reject_succeeds_under_force(): void
    {
        $firm = Firm::factory()->create();
        $this->enableExpenses($firm);
        $expense = Expense::factory()->forFirm($firm)->status(ExpenseStatus::Submitted)->create();
        $approver = FirmUser::factory()->role(FirmUserRole::BillingStaff)->forFirm($firm)->create();

        (new TenantContextService)->clearDatabaseTenantContext();

        $this->service->reject($firm, $expense, $approver, 'Missing documentation.');

        $reRead = $this->runWithFirmContext($firm, fn () => Expense::withoutGlobalScopes()->find($expense->id));

        $this->assertSame(ExpenseStatus::Rejected, $reRead->status);
    }
    ...
    public function test_expense_approval_factory_default_creation_is_safe_and_internally_consistent(): void
    {
        $approval = ExpenseApproval::factory()->create();

        $this->assertNotNull($approval->id);
        $this->assertNotNull($approval->firm_id);

        $persisted = $this->runWithFirmContext(
            $approval->firm_id,
            fn () => ExpenseApproval::withoutGlobalScopes()->with('expense')->find($approval->id),
        );

        $this->assertNotNull($persisted);
        $this->assertSame($approval->firm_id, $persisted->expense->firm_id, 'Bare factory default must not produce a cross-firm expense mismatch.');
    }
    ...
    public function test_expense_approval_row_can_reference_a_different_firms_expense_a_documented_residual_gap_not_blocked_by_rls(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        ...
        $insertedId = $this->runWithFirmContext($firmA, function () use ($firmA, $expenseB) {
            return DB::table('expense_approvals')->insertGetId($this->rowAttributes($firmA, $expenseB));
        });
        ...
        $persisted = $this->runWithFirmContext(
            $firmA,
            fn () => ExpenseApproval::withoutGlobalScopes()->find($insertedId),
        );
    ...
    public function test_migration_down_fully_restores_the_pre_checkpoint_state(): void
    {
        ...
        try {
            $row = DB::selectOne("select relrowsecurity, relforcerowsecurity from pg_class where relname = 'expense_approvals'");
            $this->assertFalse((bool) $row->relrowsecurity);
            $this->assertFalse((bool) $row->relforcerowsecurity);

            $policy = DB::selectOne(
                "select 1 from pg_policy where polrelid = 'expense_approvals'::regclass and polname = 'expense_approvals_tenant_isolation'"
            );
    ...
    public function test_migration_round_trip_affects_only_expense_approvals(): void
    {
        $coverage = new RowLevelSecurityCoverageMappingService;
        ...
        $otherTables = array_merge($sampledPrepared, [
            ...
            'accounting_export_batches', // still unprepared at this point in the batch's own migration order, must remain untouched
        ]);
     ...
     * land together as ONE combined batch. This file's own
     * app/Models/ExpenseApproval.php booted() guard is part of that
     * same, already-declared set.
     */
