...
class ContactsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_forty_three_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        $this->assertSame(161, count($actuallyForced), 'Exactly forty-three prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 25 — no more, no less.');
    ...
    public function test_no_other_tables_policy_was_changed(): void
    {
        ...
        $policy = DB::selectOne(
            "select polname from pg_policy where polrelid = 'clients'::regclass and polname = 'clients_tenant_isolation'"
        );
        ...
        $selfLookupPolicy = DB::selectOne(
            "select polname from pg_policy where polrelid = 'clients'::regclass and polname = 'clients_self_lookup'"
        );

        $this->assertNotNull($selfLookupPolicy, 'clients_self_lookup (Checkpoint 4) must be present alongside clients_tenant_isolation.');
    }
    ...
    public function test_conflict_check_service_run_finds_a_contact_match_via_search_contacts(): void
    {
        ...
        $result = $this->runWithFirmContext(
            $matter->firm,
            fn () => ConflictCheckResult::query()->where('matched_type', 'contact')->first(),
        );
