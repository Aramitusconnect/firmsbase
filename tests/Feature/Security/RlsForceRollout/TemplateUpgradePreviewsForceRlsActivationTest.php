...
class TemplateUpgradePreviewsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_twenty_six_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        // additive only, no existing assertion removed or weakened.
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['template_upgrade_previews', 'seat_allocations', 'document_requests', 'communication_consents', 'communication_consent_events', 'intake_submissions']);
        ...
        // only, no existing assertion removed or weakened.
        $this->assertSame(31, count($actuallyForced), 'Exactly thirty-one prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 13 — no more, no less (seat_allocations, document_requests, and communication_consents added on top of this batch\'s own template_upgrade_previews, plus communication_consent_events from Checkpoint 12, plus intake_submissions from Checkpoint 13).');
