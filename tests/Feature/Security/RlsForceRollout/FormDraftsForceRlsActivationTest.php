...
/**
 * FormDraftsForceRlsActivationTest — proves the FORCE ROW LEVEL
 * SECURITY activation for form_drafts (database/migrations/
 ...
 *
 * Second of the six-table, one-batch Section 39A-6 Wave 6 activation —
 * see GeneratedDocumentsForceRlsActivationTest's own docblock for the
 * full combined-batch rationale. FormDraft uses BelongsToTenant +
 * HasPublicUuid, so withoutGlobalScopes() is used throughout below.
 *
 * Unlike generated_documents, form_drafts.matter_id is NOT NULL — every
 * row must be tied to a real matter.
 *
 * This file also proves the ONE confirmed live bug this batch fixes:
 * FirmCommandCenterAggregationService::snapshot()'s formsReadyForReviewCount
 * metric previously read FormDraft with no runWithFirmContext() wrap at
 * all — now wrapped identically to every sibling metric.
 */
class FormDraftsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const MIGRATION_PATH = 'database/migrations/2026_08_27_950030_prepare_row_level_security_and_force_rls_on_form_drafts_table.php';
    ...
    public function test_form_drafts_is_discovered_by_the_forced_tables_registry(): void
    {
        $coverage = new RowLevelSecurityCoverageMappingService;

        $this->assertContains('form_drafts', $coverage->forcedTables());
    }

    public function test_form_drafts_has_row_level_security_enabled(): void
    {
        $row = DB::selectOne("select relrowsecurity from pg_class where relname = 'form_drafts'");

        $this->assertNotNull($row);
        $this->assertTrue((bool) $row->relrowsecurity);
    }

    public function test_form_drafts_has_permanent_force_row_level_security_enabled(): void
    {
        $row = DB::selectOne("select relforcerowsecurity from pg_class where relname = 'form_drafts'");

        $this->assertNotNull($row);
        $this->assertTrue(
            (bool) $row->relforcerowsecurity,
            'form_drafts must have permanent FORCE ROW LEVEL SECURITY active — this is not a transient, per-test setting.'
        );
    }
    ...
    public function test_the_policy_has_both_an_explicit_using_and_with_check_clause(): void
    {
        $row = DB::selectOne(
            "select pg_get_expr(polqual, polrelid) as using_expr, pg_get_expr(polwithcheck, polrelid) as with_check_expr
             from pg_policy where polrelid = 'form_drafts'::regclass and polname = 'form_drafts_tenant_isolation'"
        );

        $this->assertNotNull($row, 'The form_drafts_tenant_isolation policy must exist.');
    ...
    public function test_missing_tenant_context_cannot_read_form_drafts(): void
    {
        $firm = Firm::factory()->create();
        $this->createDraftForFirm($firm);

        (new TenantContextService)->clearDatabaseTenantContext();

        $this->assertSame(0, FormDraft::withoutGlobalScopes()->count());
    }

    public function test_missing_tenant_context_cannot_insert_form_drafts(): void
    {
        $firm = Firm::factory()->create();
        ...
        DB::table('form_drafts')->insert($this->rowAttributes($firm, $matter, $version));
    }
    ...
    public function test_firm_a_context_can_read_its_own_form_drafts(): void
    {
        ...
        $visibleIds = $this->runWithFirmContext(
            $firmA,
            fn () => FormDraft::withoutGlobalScopes()->pluck('id')->all(),
        );

        $this->assertSame([$draftA->id], $visibleIds);
    }

    public function test_firm_a_context_cannot_read_firm_b_form_drafts(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $this->createDraftForFirm($firmA);
        ...
        $this->assertNotContains($draftB->id, $visibleIds);
    }

    public function test_firm_a_context_can_insert_a_valid_form_draft(): void
    {
        $firmA = Firm::factory()->create();
        ...
        $insertedId = $this->runWithFirmContext(
            $firmA,
            fn () => DB::table('form_drafts')->insertGetId($this->rowAttributes($firmA, $matter, $version)),
        );

        $this->assertIsInt($insertedId);
    }

    public function test_firm_a_cannot_update_firm_b_form_draft(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $draftB = $this->createDraftForFirm($firmB);

        $affected = $this->runWithFirmContext($firmA, function () use ($draftB) {
            return DB::table('form_drafts')->where('id', $draftB->id)->update(['status' => 'ready_for_review']);
        });

        $this->assertSame(0, $affected, 'No rows should be visible/updatable — Firm A must not be able to update Firm B\'s form_drafts row.');

        $reReadAsFirmB = $this->runWithFirmContext(
            $firmB,
            fn () => FormDraft::withoutGlobalScopes()->find($draftB->id),
        );
    ...
    public function test_firm_a_cannot_delete_firm_b_form_draft(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $draftB = $this->createDraftForFirm($firmB);

        $this->runWithFirmContext($firmA, function () use ($draftB) {
            DB::table('form_drafts')->where('id', $draftB->id)->delete();
        });
        ...
        $this->assertNotNull($reReadAsFirmB, 'Firm A context must not be able to delete Firm B form_drafts.');
    }

    public function test_firm_a_cannot_insert_a_form_draft_claiming_firm_b_ownership(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        ...
        $this->runWithFirmContext($firmA, function () use ($firmB, $matterB, $version) {
            DB::table('form_drafts')->insert($this->rowAttributes($firmB, $matterB, $version));
        });
    }
    ...
    public function test_ownership_cannot_be_reassigned_across_firms(): void
    {
        ...
        $this->runWithFirmContext($firmA, function () use ($draftA, $firmB) {
            DB::table('form_drafts')->where('id', $draftA->id)->update(['firm_id' => $firmB->id]);
        });
    }
    ...
    public function test_form_draft_can_reference_a_different_firms_matter_a_documented_residual_gap_not_blocked_by_rls(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $matterB = $this->runWithFirmContext($firmB, fn () => Matter::factory()->forFirm($firmB)->create());
        ...
        $insertedId = $this->runWithFirmContext($firmA, function () use ($firmA, $matterB, $version) {
            return DB::table('form_drafts')->insertGetId($this->rowAttributes($firmA, $matterB, $version));
        });
        ...
        $persisted = $this->runWithFirmContext(
            $firmA,
            fn () => FormDraft::withoutGlobalScopes()->find($insertedId),
        );
    ...
    public function test_bare_factory_create_without_context_now_succeeds_via_the_context_hold_override(): void
    {
        ...
        $this->assertNotNull($draft->firm_id);

        $persisted = $this->runWithFirmContext(
            $draft->firm_id,
            fn () => FormDraft::withoutGlobalScopes()->with(['matter', 'generatedByFirmUser'])->find($draft->id),
        );

        $this->assertNotNull($persisted, 'A bare factory-created row must be visible under its own firm\'s context.');
        $this->assertSame($draft->firm_id, $persisted->matter->firm_id, 'Bare factory default must not produce a cross-firm matter_id mismatch.');
        $this->assertSame($draft->firm_id, $persisted->generatedByFirmUser->firm_id, 'Bare factory default must not produce a cross-firm generated_by_firm_user_id mismatch.');
    }
    ...
    // ---------------------------------------------------------------
    // Required companion fix proof — FirmCommandCenterAggregationService
    // ---------------------------------------------------------------

    /**
     * The ONE confirmed live bug in this domain: formsReadyForReviewCount
     * previously read FormDraft with no runWithFirmContext() wrap at
     * all, which would silently undercount to zero the moment
     * form_drafts is FORCE RLS'd. Now fixed — proves it under FORCE.
     */
    public function test_firm_command_center_forms_ready_for_review_count_works_under_force(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();

        $this->createDraftForFirm($firmA, ['status' => 'ready_for_review']);
        $this->createDraftForFirm($firmA, ['status' => 'ready_for_review']);
        $this->createDraftForFirm($firmA, ['status' => 'draft']);
        $this->createDraftForFirm($firmB, ['status' => 'ready_for_review']);

        $snapshot = (new FirmCommandCenterAggregationService)->snapshot($firmA);

        $this->assertSame(2, $snapshot->formsReadyForReviewCount, 'formsReadyForReviewCount must count only firm A\'s ready_for_review drafts under FORCE, never silently undercount to zero.');
    }
    ...
    public function test_migration_down_fully_restores_the_pre_checkpoint_state(): void
    {
        ...
        try {
            $row = DB::selectOne("select relrowsecurity, relforcerowsecurity from pg_class where relname = 'form_drafts'");
            $this->assertFalse((bool) $row->relrowsecurity, 'Rollback must fully disable RLS, not merely clear FORCE.');
            $this->assertFalse((bool) $row->relforcerowsecurity, 'Rollback must clear FORCE.');

            $policy = DB::selectOne(
                "select 1 from pg_policy where polrelid = 'form_drafts'::regclass and polname = 'form_drafts_tenant_isolation'"
            );
    ...
    public function test_migration_round_trip_affects_only_form_drafts(): void
    {
        $coverage = new RowLevelSecurityCoverageMappingService;
        ...
        foreach ($otherTables as $table) {
            ...
            $this->assertSame(
                ...
                "{$table}'s relrowsecurity must be unaffected by the form_drafts migration round trip."
            );
            $this->assertSame(
                (bool) $before[$table]->relforcerowsecurity,
                (bool) $after->relforcerowsecurity,
                "{$table}'s relforcerowsecurity must be unaffected by the form_drafts migration round trip."
            );
        }
    }
    ...
    private function createDraftForFirm(Firm $firm, array $overrides = []): FormDraft
    {
        $matter = $this->runWithFirmContext($firm, fn () => Matter::factory()->forFirm($firm)->create());

        return $this->runWithFirmContext($firm, fn () => FormDraft::factory()->forFirmAndMatter($firm, $matter)->create($overrides));
    }

    /**
     * @return array<string, mixed>
     */
    private function rowAttributes(Firm $firm, Matter $matter, FormTemplateVersion $version): array
    {
