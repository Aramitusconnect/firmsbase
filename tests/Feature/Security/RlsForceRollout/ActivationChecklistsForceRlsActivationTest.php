...
class ActivationChecklistsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_twenty_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        // only, no existing assertion removed or weakened.
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['activation_checklists', 'firm_activation_events', 'firm_entitlements', 'firm_entitlement_events', 'installed_template_packs', 'template_upgrade_logs', 'template_upgrade_previews', 'seat_allocations', 'document_requests', 'communication_consents']);
        ...
        // Phase C (communication_consents) for the same reason — additive
        // only, no existing assertion removed or weakened.
        $this->assertSame(29, count($actuallyForced), 'Exactly twenty-nine prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 11 — no more, no less (twenty-one after this batch\'s own Checkpoint 2 plus firm_activation_events from Checkpoint 3, plus firm_entitlements from Checkpoint 4, plus firm_entitlement_events from Checkpoint 5, plus installed_template_packs from Checkpoint 6, plus template_upgrade_logs from Checkpoint 7, plus template_upgrade_previews from Checkpoint 8, plus seat_allocations from Checkpoint 9, plus document_requests from Checkpoint 10, plus communication_consents from Checkpoint 11).');
    ...
    public function test_activation_checklists_is_isolated_independently_and_simultaneously_with_documents(): void
    {
        ...
        $documentA = $this->runWithFirmContext($firmA, fn () => \App\Models\Document::factory()->create(['firm_id' => $firmA->id]));
        $documentB = $this->runWithFirmContext($firmB, fn () => \App\Models\Document::factory()->create(['firm_id' => $firmB->id]));

        $resultA = $this->runWithFirmContext($firmA, fn () => [
            'activation_checklists' => ActivationChecklist::withoutGlobalScopes()->pluck('id')->all(),
            'documents' => \App\Models\Document::withoutGlobalScopes()->pluck('id')->all(),
        ]);
