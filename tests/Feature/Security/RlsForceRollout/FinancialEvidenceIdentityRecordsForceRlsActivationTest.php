...
/**
 * FinancialEvidenceIdentityRecordsForceRlsActivationTest — FirmsVault
 ...
 */
class FinancialEvidenceIdentityRecordsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_24_180016_prepare_row_level_security_and_force_rls_on_financial_evidence_identity_records_table.php';

    private const TABLE_MIGRATION_PATH = 'database/migrations/2026_09_24_180015_create_financial_evidence_identity_records_table.php';

    private const POLICY_NAME = 'financial_evidence_identity_records_tenant_isolation';

    private const TABLE = 'financial_evidence_identity_records';
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['owner_names_json' => json_encode(['Reassigned Name'])]));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('owner_names_json'));
        $this->assertNotSame(json_encode(['Reassigned Name']), $reReadAsFirmB);
    }
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceIdentityRecord)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceIdentityRecord::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, FirmIntegration $connection): array
    {
        return [
            ...
            'owner_names_json' => json_encode(['Jane Doe']),
            'owner_emails_json' => json_encode(['jane@example.com']),
            'owner_phones_json' => json_encode([]),
            'owner_addresses_json' => json_encode([]),
