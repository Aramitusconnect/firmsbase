...
/**
 * FinancialEvidenceReconciliationCandidatesForceRlsActivationTest —
 ...
 * `trust_ledger_entry_id` is a deliberately UNCONSTRAINED bigint (no FK
 * to trust_ledger_entries — see this table's own migration docblock),
 * so it is left null here; that column's read-only, display-only
 * nature is proven separately by
 * Tests\Feature\FinancialEvidence\FinancialEvidenceTrustLedgerFirewallTest.
 */
class FinancialEvidenceReconciliationCandidatesForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_25_190019_prepare_row_level_security_and_force_rls_on_financial_evidence_reconciliation_candidates_table.php';

    private const POLICY_NAME = 'financial_evidence_reconciliation_candidates_tenant_isolation';

    private const TABLE = 'financial_evidence_reconciliation_candidates';
    ...
    public function test_trust_ledger_entry_id_has_no_foreign_key_constraint(): void
    {
        $row = DB::selectOne(
            "select count(*) as c from pg_constraint
             where conrelid = 'financial_evidence_reconciliation_candidates'::regclass and contype = 'f'
             and conname like '%trust_ledger%'"
        );

        $this->assertSame(0, (int) $row->c, 'trust_ledger_entry_id must remain a bare, unconstrained column — never an FK into the trust domain.');
    }
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['status' => 'confirmed_match']));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('status'));
        $this->assertSame('candidate', $reReadAsFirmB);
    }
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceReconciliationCandidate)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceReconciliationCandidate::class));
    }
