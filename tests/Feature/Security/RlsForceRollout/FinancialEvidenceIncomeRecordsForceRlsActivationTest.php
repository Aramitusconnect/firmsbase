...
/**
 * FinancialEvidenceIncomeRecordsForceRlsActivationTest — FirmsVault
 ...
 */
class FinancialEvidenceIncomeRecordsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_24_180008_prepare_row_level_security_and_force_rls_on_financial_evidence_income_records_table.php';

    private const TABLE_MIGRATION_PATH = 'database/migrations/2026_09_24_180007_create_financial_evidence_income_records_table.php';

    private const POLICY_NAME = 'financial_evidence_income_records_tenant_isolation';

    private const TABLE = 'financial_evidence_income_records';
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['category' => 'reassigned']));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('category'));
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceIncomeRecord)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceIncomeRecord::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, FirmIntegration $connection): array
    {
        return [
            ...
            'income_stream_hash' => hash('sha256', Str::random(16)),
            'category' => 'salary',
            'pay_frequency' => 'biweekly',
            'summary_json' => json_encode(['average_monthly_income' => 5000]),
            'raw_json' => json_encode(['income_streams' => []]),
            'created_at' => now(),
            'updated_at' => now(),
        ];
    }
