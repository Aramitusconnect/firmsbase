...
class TimelineEventsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_fifty_one_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['ai_retrieval_indexes', 'deployment_configs', 'firm_ai_settings', 'customer_success_health_scores', 'timeline_events', 'security_events']);
        ...
        $this->assertSame(56, count($actuallyForced), 'Exactly fifty-one prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 33 — no more, no less.');
    ...
    {
        $coverage = new RowLevelSecurityCoverageMappingService();
        $forced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['ai_retrieval_indexes', 'deployment_configs', 'firm_ai_settings', 'customer_success_health_scores', 'timeline_events', 'security_events']);
