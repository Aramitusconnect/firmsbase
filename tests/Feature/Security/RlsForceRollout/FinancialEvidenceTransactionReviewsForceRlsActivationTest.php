...
/**
 * FinancialEvidenceTransactionReviewsForceRlsActivationTest —
 ...
 * Depends on a materialized `financial_evidence_transactions` row
 * (owned by the provider-core track, but reused here purely as an FK
 * dependency, never modified).
 */
class FinancialEvidenceTransactionReviewsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_25_190012_prepare_row_level_security_and_force_rls_on_financial_evidence_transaction_reviews_table.php';

    private const POLICY_NAME = 'financial_evidence_transaction_reviews_tenant_isolation';

    private const TABLE = 'financial_evidence_transaction_reviews';
    ...
    {
        $firm = Firm::factory()->create();
        [$transaction, $firmUser] = $this->makeDependencies($firm);

        (new TenantContextService)->clearDatabaseTenantContext();

        $this->expectExceptionMessageMatches('/row-level security policy/');

        DB::table(self::TABLE)->insert($this->rawRowAttributes($firm, $transaction, $firmUser));
    }
    ...
    public function test_firm_a_cannot_insert_a_row_claiming_firm_b_ownership(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        [$transactionB, $firmUserB] = $this->makeDependencies($firmB);

        $this->expectExceptionMessageMatches('/row-level security policy/');

        $this->runWithFirmContext($firmA, function () use ($firmB, $transactionB, $firmUserB) {
            DB::table(self::TABLE)->insert($this->rawRowAttributes($firmB, $transactionB, $firmUserB));
        });
    }
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['flagged' => true]));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('flagged'));
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceTransactionReview)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceTransactionReview::class));
    }
    ...
    /**
     * @return array{0: FinancialEvidenceTransaction, 1: FirmUser}
     */
    ...
    private function seedRow(Firm $firm): int
    {
        ...
        return $this->runWithFirmContext($firm, function () use ($firm, $transaction, $firmUser) {
            DB::table(self::TABLE)->insert($this->rawRowAttributes($firm, $transaction, $firmUser));
     ...
     */
    private function rawRowAttributes(Firm $firm, FinancialEvidenceTransaction $transaction, FirmUser $firmUser): array
    {
        return [
