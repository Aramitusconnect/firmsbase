...
/**
 * FinancialEvidenceDuplicateTransferFlagsForceRlsActivationTest —
 ...
 */
class FinancialEvidenceDuplicateTransferFlagsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_25_190014_prepare_row_level_security_and_force_rls_on_financial_evidence_duplicate_transfer_flags_table.php';

    private const POLICY_NAME = 'financial_evidence_duplicate_transfer_flags_tenant_isolation';

    private const TABLE = 'financial_evidence_duplicate_transfer_flags';
    ...
    {
        $firm = Firm::factory()->create();
        [$matter, $a, $b] = $this->makeDependencies($firm);

        (new TenantContextService)->clearDatabaseTenantContext();

        $this->expectExceptionMessageMatches('/row-level security policy/');

        DB::table(self::TABLE)->insert($this->rawRowAttributes($firm, $matter, $a, $b));
    }
    ...
    public function test_firm_a_cannot_insert_a_row_claiming_firm_b_ownership(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        [$matterB, $aB, $bB] = $this->makeDependencies($firmB);

        $this->expectExceptionMessageMatches('/row-level security policy/');

        $this->runWithFirmContext($firmA, function () use ($firmB, $matterB, $aB, $bB) {
            DB::table(self::TABLE)->insert($this->rawRowAttributes($firmB, $matterB, $aB, $bB));
        });
    }
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['dismissed_at' => now()]));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('dismissed_at'));
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceDuplicateTransferFlag)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceDuplicateTransferFlag::class));
    }
    ...
    /**
     * @return array{0: Matter, 1: FinancialEvidenceTransaction, 2: FinancialEvidenceTransaction}
     */
    private function makeDependencies(Firm $firm): array
    {
        return $this->runWithFirmContext($firm, function () use ($firm) {
            ...
            $a = FinancialEvidenceTransaction::query()->create([
                'firm_id' => $firm->id,
                'firm_integration_id' => $connection->id,
            ...
            $b = FinancialEvidenceTransaction::query()->create([
                'firm_id' => $firm->id,
                'firm_integration_id' => $connection->id,
                'plaid_transaction_id' => 'txn_'.Str::random(12),
                'plaid_account_id' => $account->plaid_account_id,
    ...
    private function seedRow(Firm $firm): int
    {
        ...
        return $this->runWithFirmContext($firm, function () use ($firm, $matter, $a, $b) {
            DB::table(self::TABLE)->insert($this->rawRowAttributes($firm, $matter, $a, $b));
     ...
     */
    private function rawRowAttributes(Firm $firm, Matter $matter, FinancialEvidenceTransaction $a, FinancialEvidenceTransaction $b): array
    {
        return [
