...
/**
 * FinancialEvidenceBankAccountsForceRlsActivationTest — FirmsVault Live
 * Integrations, Checkpoint 4 ("Plaid financial evidence add-on").
 * Modeled on Tests\Feature\Security\RlsForceRollout\FinancialEvidenceSnapshotsForceRlsActivationTest
 * (this checkpoint's own template) with the composite firm_id +
 * firm_integration_id foreign-key proof from
 * Tests\Feature\Security\RlsForceRollout\IntegrationProviderWebhookSubscriptionsForceRlsActivationTest,
 * since financial_evidence_bank_accounts' create migration itself
 * documents mirroring integration_provider_webhook_subscriptions'
 * activation migration. No factory exists for this immutable,
 * materializer-only model, so rows are seeded via raw DB::table()
 * inserts throughout, matching the snapshot template.
 */
class FinancialEvidenceBankAccountsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_24_180004_prepare_row_level_security_and_force_rls_on_financial_evidence_bank_accounts_table.php';

    private const TABLE_MIGRATION_PATH = 'database/migrations/2026_09_24_180003_create_financial_evidence_bank_accounts_table.php';

    private const POLICY_NAME = 'financial_evidence_bank_accounts_tenant_isolation';
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['classification' => 'reassigned']));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('classification'));
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceBankAccount)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceBankAccount::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, FirmIntegration $connection): array
    {
        return [
            ...
            'plaid_account_id' => 'acc_'.Str::random(16),
            'account_name' => 'Checking',
            'account_subtype' => 'checking',
            'mask' => '1234',
            'classification' => null,
            'raw_json' => json_encode(['account_id' => 'acc_'.Str::random(8)]),
            'created_at' => now(),
            'updated_at' => now(),
        ];
    }
