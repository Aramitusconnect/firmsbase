...
/**
 * FinancialEvidenceClientConsentsForceRlsActivationTest — FirmsVault
 * Live Integrations, Checkpoint 4. Modeled directly on
 * Tests\Feature\Integrations\IntegrationSyncCursorsForceRlsActivationTest.
 */
class FinancialEvidenceClientConsentsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_25_190004_prepare_row_level_security_and_force_rls_on_financial_evidence_client_consents_table.php';

    private const POLICY_NAME = 'financial_evidence_client_consents_tenant_isolation';
    ...
    public function test_missing_tenant_context_cannot_insert(): void
    {
        ...
        DB::table(self::TABLE)->insert($this->rawRowAttributes($firm, $client, $matter));
    }
    ...
    public function test_firm_a_cannot_insert_a_row_claiming_firm_b_ownership(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $client = Client::factory()->forFirm($firmB)->create();
        $matter = $this->runWithFirmContext($firmB, fn () => Matter::factory()->forFirm($firmB)->forClient($client)->create());

        $this->expectExceptionMessageMatches('/row-level security policy/');

        $this->runWithFirmContext($firmA, function () use ($firmB, $client, $matter) {
            DB::table(self::TABLE)->insert($this->rawRowAttributes($firmB, $client, $matter));
        });
    }
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['ip_address' => '9.9.9.9']));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('ip_address'));
        $this->assertSame('203.0.113.5', $reReadAsFirmB);
    }
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceClientConsent)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceClientConsent::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, Client $client, Matter $matter): array
    {
        return [
            ...
            'client_id' => $client->id,
            'matter_id' => $matter->id,
            'matter_request_id' => null,
            'firm_integration_id' => null,
            'granted_products_json' => json_encode(['bank_account', 'transaction']),
            'granted_at' => now(),
            'declined_at' => null,
