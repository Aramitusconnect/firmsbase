...
class IntakeSubmissionsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_thirty_one_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        // the same reason — additive only, no existing assertion
        // removed or weakened.
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['intake_submissions', 'matter_readiness_scores', 'readiness_score_events', 'tenant_encryption_keys']);
        ...
        $this->assertSame(34, count($actuallyForced), 'Exactly thirty-four prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 13 — no more, no less. Narrowly updated again for Section 39A-3L, Checkpoint 14 (matter_readiness_scores added on top of the prior thirty-one), again for Checkpoint 15 (readiness_score_events added on top of the prior thirty-two), and again for Checkpoint 16 (tenant_encryption_keys added on top of the prior thirty-three).');
    ...
    public function test_no_unrelated_prepared_table_became_force_enabled(): void
    {
        ...
        // the same reason — additive only, no existing assertion
        // removed or weakened.
        $forced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['intake_submissions', 'matter_readiness_scores', 'readiness_score_events', 'tenant_encryption_keys']);
