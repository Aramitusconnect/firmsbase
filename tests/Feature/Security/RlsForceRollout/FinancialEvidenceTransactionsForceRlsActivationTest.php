...
/**
 * FinancialEvidenceTransactionsForceRlsActivationTest — FirmsVault
 * Live Integrations, Checkpoint 4 ("Plaid financial evidence
 * add-on"). Modeled on
 * Tests\Feature\Security\RlsForceRollout\FinancialEvidenceBankAccountsForceRlsActivationTest,
 * this table's own create migration documenting a byte-for-byte
 * identical RLS activation shape. `bank_account_id` is left null in
 * every seeded row here — it is a nullable, best-effort local FK (see
 * the create migration's own docblock: Transactions and Auth are
 * pulled independently, no ordering guarantee), irrelevant to the
 * tenant-isolation properties this file proves.
 */
class FinancialEvidenceTransactionsForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_24_180006_prepare_row_level_security_and_force_rls_on_financial_evidence_transactions_table.php';

    private const TABLE_MIGRATION_PATH = 'database/migrations/2026_09_24_180005_create_financial_evidence_transactions_table.php';

    private const POLICY_NAME = 'financial_evidence_transactions_tenant_isolation';

    private const TABLE = 'financial_evidence_transactions';
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['merchant_name' => 'reassigned']));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('merchant_name'));
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceTransaction)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceTransaction::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, FirmIntegration $connection): array
    {
        return [
            ...
            'category_json' => json_encode(['primary' => 'FOOD_AND_DRINK']),
            'pending' => false,
            'provider_retrieved_at' => now(),
            'raw_json' => json_encode(['transaction_id' => 'txn_'.Str::random(8)]),
            'created_at' => now(),
            'updated_at' => now(),
        ];
    }
