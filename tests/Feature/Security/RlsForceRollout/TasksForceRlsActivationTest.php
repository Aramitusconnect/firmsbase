...
class TasksForceRlsActivationTest extends TestCase
{
    ...
    public function test_tasks_has_row_level_security_enabled(): void
    {
        $row = DB::selectOne("select relrowsecurity from pg_class where relname = 'tasks'");

        $this->assertNotNull($row);
        $this->assertTrue((bool) $row->relrowsecurity);
    }

    public function test_tasks_has_permanent_force_row_level_security_enabled(): void
    {
        ...
        $this->assertTrue(
            ...
            'tasks must have permanent FORCE ROW LEVEL SECURITY active — this is not a transient, per-test setting.'
        );
    }

    public function test_missing_tenant_context_cannot_read_tasks(): void
    {
        $firm = Firm::factory()->create();
        Task::factory()->create(['firm_id' => $firm->id]);
    ...
    public function test_missing_tenant_context_cannot_write_tasks(): void
    {
        $firm = Firm::factory()->create();
    ...
    public function test_firm_a_context_can_read_its_own_tasks(): void
    {
        ...
        $visibleIds = $this->runWithFirmContext(
            $firmA,
            fn () => Task::withoutGlobalScopes()->pluck('id')->all(),
        );

        $this->assertSame([$taskA->id], $visibleIds);
    }

    public function test_firm_a_context_cannot_read_firm_b_tasks(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        Task::factory()->create(['firm_id' => $firmA->id]);
    ...
    public function test_firm_a_context_cannot_update_firm_b_tasks(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $taskB = Task::factory()->create(['firm_id' => $firmB->id, 'title' => 'Original Title']);

        $this->runWithFirmContext($firmA, function () use ($taskB) {
            DB::table('tasks')->where('id', $taskB->id)->update(['title' => 'Hacked Title']);
        });

        $reReadAsFirmB = $this->runWithFirmContext(
            $firmB,
            fn () => Task::withoutGlobalScopes()->find($taskB->id),
        );
    ...
    public function test_firm_a_context_cannot_delete_firm_b_tasks(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
        $taskB = Task::factory()->create(['firm_id' => $firmB->id]);
        ...
        $this->runWithFirmContext($firmA, function () use ($taskB) {
            DB::table('tasks')->where('id', $taskB->id)->delete();
        });
        ...
        $this->assertNotNull($reReadAsFirmB, 'Firm A context must not be able to delete Firm B tasks.');
    }

    public function test_firm_a_context_cannot_insert_a_task_claiming_firm_b_ownership(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
    ...
    public function test_tenant_context_clears_after_success(): void
    {
        ...
        $this->runWithFirmContext($firm, fn () => Task::factory()->create(['firm_id' => $firm->id]));
    ...
    {
        $migration = require base_path('database/migrations/2026_08_03_900001_force_rls_on_tasks_table.php');

        $migration->down();

        try {
            $row = DB::selectOne("select relrowsecurity, relforcerowsecurity from pg_class where relname = 'tasks'");
    ...
    /**
     * All five staged batches (clients, firm_users, documents,
     * deadlines, tasks) must be independently force-active and
     * independently isolated at the same time — proof this batch did
     * not weaken or interfere with Section 39A-3A/39A-3B/39A-3C/39A-3D's
     * own enforcement.
     */
    public function test_clients_firm_users_documents_deadlines_and_tasks_are_isolated_independently_and_simultaneously(): void
    {
        $firmA = Firm::factory()->create();
        $firmB = Firm::factory()->create();
