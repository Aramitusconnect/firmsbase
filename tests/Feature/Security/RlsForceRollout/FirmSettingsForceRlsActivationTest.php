...
class FirmSettingsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_thirty_six_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        // or weakened.
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['firm_settings', 'firm_licenses', 'time_tracking_sessions', 'time_entries', 'payment_plans', 'payment_plan_events']);
        $actuallyForced = [];
        ...
        // same reason — additive only, no existing assertion removed
        // or weakened.
        $this->assertSame(41, count($actuallyForced), 'Exactly thirty-six prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 18 — no more, no less.');
    ...
    public function test_no_unrelated_prepared_table_became_force_enabled(): void
    {
        ...
        // same reason — additive only, no existing assertion removed
        // or weakened.
        $forced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['firm_settings', 'firm_licenses', 'time_tracking_sessions', 'time_entries', 'payment_plans', 'payment_plan_events']);
        foreach ($coverage->preparedTables() as $table) {
