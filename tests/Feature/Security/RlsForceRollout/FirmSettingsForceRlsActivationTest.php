...
class FirmSettingsForceRlsActivationTest extends TestCase
{
    ...
    public function test_exactly_thirty_six_prepared_tables_are_force_row_level_security_enabled(): void
    {
        ...
        $expectedForced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['firm_settings']);
        ...
        $this->assertSame(36, count($actuallyForced), 'Exactly thirty-six prepared tables must be FORCE RLS enabled after Section 39A-3L, Checkpoint 18 — no more, no less.');
    ...
    {
        $coverage = new RowLevelSecurityCoverageMappingService();
        $forced = array_merge(self::PREVIOUSLY_FORCED_TABLES, ['firm_settings']);
