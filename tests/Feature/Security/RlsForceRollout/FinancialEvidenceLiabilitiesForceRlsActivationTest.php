...
/**
 * FinancialEvidenceLiabilitiesForceRlsActivationTest — FirmsVault Live
 ...
 */
class FinancialEvidenceLiabilitiesForceRlsActivationTest extends TestCase
{
    use RefreshDatabase;

    private const RLS_MIGRATION_PATH = 'database/migrations/2026_09_24_180010_prepare_row_level_security_and_force_rls_on_financial_evidence_liabilities_table.php';

    private const TABLE_MIGRATION_PATH = 'database/migrations/2026_09_24_180009_create_financial_evidence_liabilities_table.php';

    private const POLICY_NAME = 'financial_evidence_liabilities_tenant_isolation';

    private const TABLE = 'financial_evidence_liabilities';
    ...
    public function test_firm_a_cannot_update_firm_b_row(): void
    {
        ...
        $affected = $this->runWithFirmContext($firmA, fn () => DB::table(self::TABLE)->where('id', $idB)->update(['type_specific_json' => json_encode(['reassigned' => true])]));
        $this->assertSame(0, $affected);

        $reReadAsFirmB = $this->runWithFirmContext($firmB, fn () => DB::table(self::TABLE)->where('id', $idB)->value('type_specific_json'));
        $this->assertNotSame(json_encode(['reassigned' => true]), $reReadAsFirmB);
    }
    ...
    {
        $this->assertSame(self::TABLE, (new FinancialEvidenceLiability)->getTable());
    }

    public function test_model_uses_belongs_to_tenant_trait(): void
    {
        $this->assertArrayHasKey(BelongsToTenant::class, class_uses_recursive(FinancialEvidenceLiability::class));
    }
    ...
    private function rawRowAttributes(Firm $firm, FirmIntegration $connection): array
    {
        return [
            ...
            'type_specific_json' => json_encode(['is_overdue' => false]),
