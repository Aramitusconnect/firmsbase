...
abstract class TestCase extends BaseTestCase
{
    ...
    /**
     * Mission 1 (Domain & Security Boundary Architecture) test helpers —
     * absolute URLs on each canonical hostname, for use with $this->get()/
     * ->post()/etc. Laravel's HTTP test client performs no real network
     * lookup for an absolute URL; it only sets the in-process request's
     * Host header, so these never depend on public or local DNS. Reading
     * through CanonicalUrlService (the same authority the application
     * itself uses) rather than hardcoding a hostname here keeps tests
     * correct if the config/hosts.php defaults ever change.
     */
