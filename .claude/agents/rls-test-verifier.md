...
- Never run `migrate:fresh`, `migrate:refresh`, or `db:wipe` under any circumstance. Never create or destroy a database yourself — request one from the coordinator via `tools/rls-test/create-disposable-db.sh`. Never target `firmsbase`, `firmsbase_test`, or any persistent/quarantined/sibling-mission database; run everything through `tools/rls-test/run-artisan-test.sh`.
...
4. Run `php -l` on every changed file and `git diff --check` directly (not database operations, no wrapper needed). For everything else, use exclusively `tools/rls-test/run-artisan-test.sh <disposable_db_name> -- ...` against a disposable database the coordinator created and handed you for this task — **never create or destroy that database yourself, and never target a persistent database**: `-- migrate --force` to build schema inside that already-empty disposable database (never `migrate:fresh` — the database is disposable and already empty, so a bare `migrate` is both sufficient and safe, and `migrate:fresh`/`migrate:refresh`/`db:wipe` are permanently prohibited in this role); the new test in isolation; every required focused filter with `--stop-on-failure`; the full suite twice (with and without `--stop-on-failure`), each run against its own freshly created disposable database — request a second one from the coordinator rather than reusing the first.
