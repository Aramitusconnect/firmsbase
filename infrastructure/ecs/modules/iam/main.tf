...
resource "aws_iam_role" "task_execution" {
  ...
  permissions_boundary = var.task_execution_permissions_boundary_arn
  tags                 = var.tags
...
locals {
  ...
  # Boundary per task role, resolved once so the role resource below indexes a
  # map that is guaranteed to have every role key. When the caller supplies
  # boundaries the variable's own validation has already proven the key set is
  # exactly task_role_names, so this index can never fall back to a default —
  # there is deliberately no lookup()/try() here. An unmapped role key must
  # fail the plan, not silently acquire a null boundary.
  task_permissions_boundary_arns = length(var.task_permissions_boundary_arns) > 0 ? var.task_permissions_boundary_arns : {
    for key in local.task_role_names : key => null
  }
}
...
resource "aws_iam_role" "task" {
  ...
  assume_role_policy   = data.aws_iam_policy_document.ecs_tasks_assume_role.json
  permissions_boundary = local.task_permissions_boundary_arns[each.key]
  tags                 = var.tags
}
