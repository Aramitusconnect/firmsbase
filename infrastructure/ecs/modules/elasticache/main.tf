...
resource "aws_security_group" "redis" {
  ...
  # var.security_group_adoption_tags carries this one resource's pre-
  # Terraform-adoption legacy tag (e.g. {"firmsbase-staging-redis-sg" = ""}
  ...
  tags = merge(var.tags, var.security_group_adoption_tags, { Name = "${var.name_prefix}-redis" })

  lifecycle {
    create_before_destroy = true

    # tags/tags_all are deliberately NOT ignored here (unlike the sibling
    # resources below): the legacy pre-Terraform tag is now explicitly
    # modeled via var.security_group_adoption_tags above, merged with the
    # Name tag and the provider's default_tags, so a routine plan can
    # verify the live security group's tags match config exactly rather
    # than masking any drift. Previously this tag was protected only by an
    # ignore_changes entry, which — per docs/ecs/state-adoption-plan.md —
    # does not reliably survive a real apply's tag-reconciliation call.
    #
