...
class ComplianceGapRegistryService
{
    private const GAP_ITEMS = [
        [
            ...
            'description' => 'PostgreSQL row-level security is not enforced schema-wide. SET LOCAL app.current_firm_id context wiring (TenantContextService) does exist and is exercised by every FORCE-activated table\'s write paths — it is not a missing mechanism. What remains genuinely open: some prepared tables (ENABLE ROW LEVEL SECURITY + CREATE POLICY) still lack FORCE ROW LEVEL SECURITY, leaving them inert for the app\'s own database connection (table-owner role is exempt from non-forced RLS). RLS preparation itself is also incomplete: additional tenant-owned tables discovered by inventory sweeps remain without any prepared RLS policy at all. See RowLevelSecurityCoverageMappingService for the current prepared, forced, prepared-but-unforced, and uncovered table counts — this description intentionally does not hardcode them, since every rollout batch changes them. Both the remaining prepared-but-unforced tables and the uncovered tables must be addressed before FORCE ROW LEVEL SECURITY / SET LOCAL enforcement can be considered active for the whole schema. Section 28 test-coverage impact: the master plan\'s required "broken query scope caught by row-level security" test group cannot honestly be classified Implemented while this same blocker persists — tests/Feature/Tenancy/RowLevelSecurityPreparationTest.php explicitly asserts FORCE ROW LEVEL SECURITY is NOT enabled for its scoped table set, proving there is still nothing at the database layer to catch a broken/bypassed application-layer scope for the tables that remain unforced. See TestCoverageMappingService::byKey(\'tenant_isolation_broken_scope_caught_by_rls\'). Section 29 deployment-mode impact: SaaS is the deployment mode most exposed by this gap, since SaaS firms share the same database/schema and rely on tenant isolation the most heavily — see DeploymentModeCoverageMappingService::byKey(\'saas_firm_isolation_rls_defense_in_depth\').',
