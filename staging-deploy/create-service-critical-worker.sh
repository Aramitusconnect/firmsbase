...
# 603013471426) only after the "worker" service is confirmed running and
# consuming jobs correctly.
#
# Prerequisite: firmsbase-staging-critical-worker.json already registered.
set -euo pipefail

aws ecs create-service \
  --cluster firmsbase-staging-cluster \
  --service-name critical-worker \
  --task-definition firmsbase-staging-critical-worker \
  --desired-count 1 \
  --launch-type FARGATE \
...
# No --load-balancers: this task def (WORKER_QUEUES=trust only) has no port
# mapping and must never be attached to the ALB target group.
#
# Post-creation verification (see runtime-verification-commands.sh): confirm
# via CloudWatch logs that this service is consuming ONLY the "trust" queue,
# never "default"/"documents"/etc., to prove queue isolation from the
