...
- [ ] Required secrets (`APP_KEY`, `DB_PASSWORD`, `REDIS_PASSWORD`) already exist in Secrets Manager at the ARNs the target Terraform environment references. No integration-specific secret exists to provision today — only TestProvider is registered, and it is synthetic with zero network calls. A real provider's OAuth client id/secret or webhook signing secret would flow through the existing `IntegrationCredentialService` path — never a new mechanism.
