...
- **Status:** open.
- **Authoritative live count:** `RowLevelSecurityCoverageMappingService::missingPreparedTables()`. This document intentionally does not enumerate all 31 table names, to avoid a second stale hardcoded list — the code-side service is the source of truth for the exact list at any given time; sibling agent 1A owns keeping that list current as classification work proceeds.
...
|---|---|---|---|
| Uncovered tenant-owned tables | 31 | open, ongoing | this document, §1 |
