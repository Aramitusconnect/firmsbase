...
## 1. Primary ongoing gap: 0 uncovered tenant-owned tables — CLOSED by Section 39A-5 Wave 11

**Count: 0** (was 61 as of the Wave 0/1 snapshot this document was originally written against; Section 39A-5, Checkpoint 1 closed one of them — `customer_success_health_scores`; Section 39A-5 Wave 1 closed three more — `ai_retrieval_indexes`, `deployment_configs`, `firm_ai_settings`; Section 39A-5 Wave 2 closed four more — `email_visibility_rules`, `private_enterprise_settings`, `matter_expenses`, `email_message_links`; Section 39A-5 Wave 3 closed five more — `ai_usage_events`, `ai_tool_actions`, `firm_ai_provider_keys`, `ai_approval_requests`, `ai_approval_events`; Section 39A-5 Wave 4 closed seven more — `chart_of_accounts`, `expense_categories`, `expenses`, `expense_receipts`, `expense_approvals`, `accounting_export_batches`, `accounting_export_lines`; Section 39A-5 Wave 5 closed four more — `email_accounts`, `email_messages`, `email_attachments`, `email_sync_events`; Section 39A-5 Wave 6 closed six more — `generated_documents`, `form_drafts`, `generated_document_events`, `form_review_events`, `document_hashes`, `pdf_view_events`; Section 39A-5 Wave 7 closed four more — `signature_requests`, `signature_request_recipients`, `signature_events`, `signature_certificates`; Section 39A-5 Wave 8 closed six more — `legal_holds`, `deletion_requests`, `key_destruction_requests`, `support_access_requests`, `support_access_sessions`, `deployment_health_checks`; Section 39A-5 Wave 9 closed six more — `export_jobs`, `migration_projects`, `import_batches`, `implementation_projects`, `fleet_migration_instance_status`, `offboarding_requests`; Section 39A-5 Wave 10 closed ten more — `trust_accounts`, `trust_ledgers`, `trust_balances`, `matter_trust_balances`, `trust_ledger_entries`, `trust_approval_events`, `trust_chargeback_events`, `trust_reconciliations`, `trust_refund_requests`, `trust_transfer_requests` — by giving each a real RLS policy and FORCE activation in the same batch; Section 39A-5 Wave 11 closed the final five — `webhook_subscriptions`, `webhook_events`, `webhook_secrets`, `webhook_deliveries`, `webhook_delivery_attempts` — by the same combined prepare-and-force shape, and additionally fixed two feature-breaking gaps found during that wave's caller/dependency analysis (a decoy tenant-context wrap in `WebhookEventRecorderService::record()` and a completely unwrapped `WebhookDispatchJob::handle()`) that would otherwise have silently broken webhook delivery the moment FORCE RLS activated; see `docs/governance/future-table-requirements.md` for the closure shape every wave followed). These were tenant-owned tables with **no RLS preparation at all** — no `ENABLE ROW LEVEL SECURITY`, no policy. The original 61 was a corrected, larger figure than the previously-documented "43 uncovered" snapshot from 2026-07-10 (see `docs/governance/section-40-limited-pilot-safety-gate.md` and `docs/security/section-39a-post-pilot-critical-rls-checkpoint.md` for the corrected historical record). The growth from 43 to 61 was a **scoping correction**, not a regression: a later Wave 0 inventory sweep found more true tenant-owned tables than the original 39A-4 scope had captured — no table that was previously covered lost its coverage.

- **Severity:** High — matched the severity already assigned to `rls_prepared_not_enforced` in `ComplianceGapRegistryService` while this gap was open.
- **Suggested owning gate:** the 39A-4-successor classification/preparation effort (Wave 1 through Wave 11) — now complete.
- **Status:** CLOSED as of Section 39A-5 Wave 11 (the eleventh and final coordinated multi-table wave). Every tenant-owned table identified across the Wave 0/1 classification sweep now has real RLS preparation and FORCE activation. Closing/updating the `rls_prepared_not_enforced` gap's status in `ComplianceGapRegistryService` itself, and any related severity reassessment, is explicitly **out of scope for this wave's own design/implementation pass** and should be a separate, human-reviewed follow-up — this document only registers that the underlying table backlog is now empty, per this rollout's established practice of not silently expanding scope beyond what was authorized.
- **Authoritative live count:** `RowLevelSecurityCoverageMappingService::missingPreparedTables()` — now empty. A future genuinely-new tenant-owned table (e.g. from an as-yet-unbuilt feature) would still need to be classified and added to `MISSING_PREPARED_TABLES` first, per that service's own docblock; an empty count today does not retire this registry entry as a concept, only closes the backlog known as of this rollout.
- **What "closing" this gap required, per table:** classification (tenant-owned, confirmed ownership path), an RLS preparation migration (`ENABLE ROW LEVEL SECURITY` + policy), and a FORCE migration plus activation test — see `docs/governance/future-table-requirements.md` for the full requirement shape every table crossing from uncovered to covered followed.
...
|---|---|---|---|
| Uncovered tenant-owned tables | 0 (closed) | closed as of 39A-5 Wave 11 | this document, §1 |
