...
Seat enforcement is wired in as specified: `invite()` checks `SeatEnforcementService::canInvite()` for the invited role's `effectiveSeatClass()` and throws `FirmSeatLimitExceededException` (a clear, UI-surfaced message) rather than silently ignoring the limit. **Confirmed pre-existing gap, not introduced here:** no production code creates a `seat_allocations` row for a firm today (`FirmProvisioningService` never calls `SeatAllocationService`), so a freshly-provisioned firm has zero allocated seats and every invite will be seat-blocked until a (separately tracked) seat-provisioning step allocates seats via `SeatAllocationService::allocateDirect()`/`allocateFromPool()`.
...
Tests: `tests/Feature/FirmTeam/FirmUserInvitationServiceTest.php` (11), `tests/Feature/FirmTeam/FirmUserResourceAccessTest.php` (15, includes the RLS regression checklist — own-firm-only list, no cross-firm leak, direct-URL-to-foreign-record blocked), plus `FirmUserResource::class` added to `FirmWorkspaceCompletenessGuardTest`'s roster. All 26 new + 22 completeness-guard tests pass; no regression in `ResetPasswordInvitationAcceptanceTest`/`ClientCrm`.

Files: `app/Services/FirmUserInvitationService.php`, `app/Services/FirmMembershipAccessPolicyService.php`, `app/Exceptions/FirmSeatLimitExceededException.php`, `app/Exceptions/LastFirmOwnerRemovalException.php`, `app/Filament/Firm/Resources/FirmUserResource.php` + `FirmUserResource/{Pages,Actions}/*`. The platform-admin `App\Filament\Resources\FirmUserResource`, `App\Policies\FirmUserPolicy`, and `FirmProvisioningService` were **not** modified.
