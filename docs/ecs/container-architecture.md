...
There is exactly **one** application image (`Dockerfile` at repo root). Every ECS role — web, general queue worker, critical queue worker, scheduler, migration task, maintenance task, SES bounce/complaint consumer (`ses-consumer`) — runs the **same image digest**, differing only in:
...
Only the **web** role serves either endpoint (they're HTTP routes). Worker/scheduler/migration/maintenance/ses-consumer tasks have no HTTP listener at all — their "liveness" for ECS purposes is simply "the process is still running" (ECS's own container-level process-exit detection), documented per-role in [Phase 8 task definitions](infrastructure-architecture.md).
...
- **ses-consumer**: `docker/commands/ses-consumer.sh` execs `php artisan ses:consume-events`. Unlike worker/scheduler, this command installs its *own* `pcntl_signal()`/`pcntl_async_signals()` handlers directly (rather than relying on a Laravel framework component's built-in handling — `ses:consume-events` is a plain `Illuminate\Console\Command`, not `queue:work`/`schedule:work`) — see [graceful-shutdown.md](graceful-shutdown.md) "SES consumer" for the exact flag/loop-check design. Same net behavior as the other long-running roles: a message already being processed finishes and is deleted before a pending shutdown is honored; no new receive cycle or new message starts afterward.
...
1. **Resolves the role first** (`$1`), before any validation, so validation can be role-specific.
2. **Fail fast on missing required environment variables** — a fixed list (`APP_KEY`, `APP_ENV`, `DB_CONNECTION`, `DB_HOST`, `DB_DATABASE`, `DB_USERNAME`, `DB_PASSWORD`, plus `REDIS_HOST` when `CACHE_STORE`/`SESSION_DRIVER`/`QUEUE_CONNECTION` is `redis`) is checked with `: "${VAR:?required}"`; any missing variable aborts the container immediately with a clear message on stderr, before any Laravel bootstrap runs. This is deliberately shell-level, not a `php artisan` command, so it fails in milliseconds without needing PHP/Composer autoload to even load. Two additional variables are required **only** for specific roles: `SES_EVENTS_QUEUE_URL` for `ses-consumer` only, and `PLATFORM_NOTIFICATIONS_RECIPIENT_FINGERPRINT_HMAC_KEY` for `web` and `ses-consumer` only (see [iam-matrix.md](iam-matrix.md) — worker/critical-worker/scheduler/migrate/maintenance never call the code path that needs it, and are not made to depend on a secret they don't need).
3. **Does not run migrations automatically** on web/worker/scheduler/ses-consumer startup — migrations only ever run via the dedicated `docker/commands/migrate.sh` command, invoked as its own ECS one-off task. A web task restarting (deploy, scale-out, task replacement) never touches schema.
4. **Does not cache configuration until required secrets are confirmed present** — `config:cache`/`route:cache`/`view:cache` are *not* run in the entrypoint at all in this branch; they are run once at image-build time in the Dockerfile's runtime stage against build-time-safe defaults are **not** baked in (no secrets exist at build time) — instead, `bootstrap/cache/*.php` is left absent from the image and Laravel falls back to reading `config/*.php` + real env at request time. (Baking `config:cache` into the image would require build-time secrets, which the mission explicitly forbids copying into the image; running it in the entrypoint after the env-var check would add per-task-start latency without much benefit at this app's current size. This trade-off is revisited in [staging-readiness-report.md](staging-readiness-report.md) as a "Ready with configuration" item — enabling build-time `config:cache` against **non-secret** config only, once the config tree is confirmed to contain no secret defaults, is a safe follow-up.)
5. **Supports role-specific commands** via `exec "docker/commands/${role}.sh"`, where `role` is the ECS task's `command[0]` (`web`/`worker`/`scheduler`/`migrate`/`maintenance`/`ses-consumer`). Unknown role values exit non-zero with a clear error rather than silently doing nothing.
6. **Logs to stdout/stderr only** — nothing in the entrypoint or command scripts redirects output to a file.
7. **No hidden background daemons** — no `&`-backgrounded process anywhere in the entrypoint or command scripts; every script's last action is `exec`.
