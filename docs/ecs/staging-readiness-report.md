...
| **Terraform**: downloaded Terraform 1.9.8 directly (network access was available even though the binary wasn't preinstalled), ran `fmt`, `init -backend=false`, `validate` (Success), and `plan` with fake AWS credentials (got past all variable/resource-attribute evaluation, failed only at the real STS credential check — proof nothing could have been created) | Pass — two real bugs found and fixed in the process (security-group rule description character restrictions; `auth_token` requiring `aws_elasticache_replication_group` not `aws_elasticache_cluster`) |
...
| Terraform remote state backend (S3+DynamoDB or Terraform Cloud) | Requires human approval — not chosen by this branch | `infrastructure/ecs/environments/staging/versions.tf` |
