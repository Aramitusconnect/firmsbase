...
| `SqsClient`'s HTTP `connect_timeout` | 5s (`app/Providers/AppServiceProvider.php`) | Bounds only the TCP/TLS handshake phase — generous for a reachable AWS endpoint, fast-fails on an unreachable one (see "Intentional shutdown vs. genuine failure" below). |
| `SqsClient`'s HTTP `timeout` (total request) | `wait_time_seconds + 10` = 30s by default | Bounds the *whole* `receiveMessage()` call including the long-poll wait itself. Deliberately derived from the same normalized `wait_time_seconds` value the call itself uses (`AppServiceProvider::normalizeSesEventsWaitTimeSeconds()`, clamped to SQS's own 0-20 range, falling back to 20 on invalid input) — **always exceeds** `WaitTimeSeconds` by design, so a legitimate near-20s empty long poll is never cut short. |
| ECS `ses-consumer` task `stopTimeout` | 45s (`var.ses_consumer_stop_timeout`) | Must exceed the SqsClient's own derived total timeout (30s by default) with real margin for the consumer's own shutdown-flag check and any in-flight message's processing/delete — enforced by a Terraform cross-variable validation (`ses_consumer_stop_timeout > ses_events_wait_time_seconds + 10 + 5`), not just a comment, so `terraform plan`/`apply` fails outright rather than silently shipping a value that could force ECS to SIGKILL the task before its own clean exit fires. Comfortably under ECS's 120s ceiling. |

### Intentional shutdown vs. genuine failure

A signal arriving while `receiveMessage()` is blocked (mid network call) surfaces to `ConsumeSesEventsCommand` as a thrown `AwsException` once the SDK's own bounded timeouts above expire (or, in the common case, as soon as pcntl's async signal delivery interrupts the blocking call directly) — from the command's perspective these look identical to a *genuine* SQS outage. The command distinguishes them explicitly: if `$shouldStop` was already set (a signal was actually received) when that exception is caught, it is treated as **intentional shutdown** — logged distinctly (no stack trace, no error-level log), and the command exits `Command::SUCCESS` (exit code 0), not `Command::FAILURE`. If no signal was received, the exact same exception is a **genuine failure** and the command still exits `Command::FAILURE` (exit code 1), preserving the existing retry/redrive behavior for a real outage. Verified directly: real `SIGTERM`/`SIGINT` delivery to the process while blocked in a receive call exits 0; the identical blocking failure with no signal sent still exits 1 (see `tests/Feature/Notifications/ConsumeSesEventsCommandTest.php` and the container-level smoke test results in this mission's final report).
