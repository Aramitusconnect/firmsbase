 ...
 * batch's four approved prepared tables: consultation_outcomes.
 *
 * consultation_outcomes is a small firm-scoped lookup table (code/
 * name/is_active) with no nested tenant-owned foreign keys of its
 * own, so there is no cross-firm-mismatch class of bug possible in
 * its factory beyond the bare firm_id itself — the only change
 * required alongside this migration is ConsultationOutcomeFactory's
 * context-hold create() retrofit (Section 39A-3A/39A-3F pattern) so
 * a bare ConsultationOutcome::factory()->create() can still insert
 * once FORCE is active.
 *
...
{
    private const TABLE = 'consultation_outcomes';
