...
class SignatureCertificateFactory extends Factory
{
     ...
     * its OWN parent signature_request — one authoritative request is
     ...
     * already-correct logic below. The nested DocumentHash::factory()
     * call self-handles its own context via DocumentHashFactory's own
     * pre-existing context-hold create() override. A bare
     * signature_certificates row whose firm_id disagrees with its own
     * signature_request_id's parent firm is exactly the transitive
    ...
    public function definition(): array
    {
        ...
        return [
            ...
            'document_hash_id' => DocumentHash::factory()->create(['firm_id' => $request->firm_id])->id,
            'generated_at' => now(),
        ];
    }
